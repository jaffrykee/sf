...
namespace SFResConfigReader
{
	...
	bool readXMLNode(CComPtr<IXmlReader> pReader, UINT tabCount[], SFResScene* pResScene)
	{
		HRESULT hr = S_OK;
		LPCWSTR name;
		LPCWSTR value;
	...
	/*XML解析主逻辑入口*/
	/*
		玩家解析，nodeMax为RNP_MAX
	*/
	bool readFromXML(string xmlPath, SFResPlayer* pRes)
	{
		HRESULT hr = S_OK;
		...
		const UINT nodeMax = RNP_MAX;
		bool nodeSw[nodeMax] = { false };
		bool nodeHad[nodeMax] = { false };
		UINT nodeCount[nodeMax] = { 0 };
		vector<bool>& nodeIsOnly = g_pConf->m_aResPlayerXMLNodeIsOnly;
		vector<wstring>& nodeName = g_pConf->m_aResPlayerXMLNodeName;
		...
		POLL_XML_NODE_BEGIN(RNP_PLY_INF)
			POLL_XML_NODE_BEGIN(RNP_SKN_TBL)
				POLL_XML_NODE_BEGIN(RNP_SKN)
				POLL_XML_NODE_END(RNP_SKN)
			POLL_XML_NODE_END(RNP_SKN_TBL)
			POLL_XML_NODE_BEGIN(RNP_SKL_TBL)
				POLL_XML_NODE_BEGIN(RNP_SKL)
					POLL_XML_NODE_BEGIN(RNP_OBJ_TBL)
						POLL_XML_NODE_BEGIN(RNP_OBJ)
							POLL_XML_NODE_BEGIN(RNP_FRM_TBL)
								POLL_XML_NODE_BEGIN(RNP_FRM)
									POLL_XML_NODE_BEGIN(RNP_FRM_POI)
									POLL_XML_NODE_END(RNP_FRM_POI)
									POLL_XML_NODE_BEGIN(RNP_BOX_TBL)
										POLL_XML_NODE_BEGIN(RNP_BOX)
											POLL_XML_NODE_BEGIN(RNP_BOX_RCT)
											POLL_XML_NODE_END(RNP_BOX_RCT)
										POLL_XML_NODE_END(RNP_BOX)
									POLL_XML_NODE_END(RNP_BOX_TBL)
								POLL_XML_NODE_END(RNP_FRM)
							POLL_XML_NODE_END(RNP_FRM_TBL)
						POLL_XML_NODE_END(RNP_OBJ)
					POLL_XML_NODE_END(RNP_OBJ_TBL)
				POLL_XML_NODE_END(RNP_SKL)
			POLL_XML_NODE_END(RNP_SKL_TBL)
		POLL_XML_NODE_END(RNP_PLY_INF)

		return true;
	}
	
	/*
		场景解析，nodeMax为RNS_MAX
	*/
	bool readFromXML(string xmlPath, SFResScene* pRes)
	{
		HRESULT hr = S_OK;
		CComPtr<IStream> pFileStream;
		CComPtr<IXmlReader> pReader;
		...
		LPCWSTR name;
		WCHAR trueName[NODE_NAME_MAX] = { 0 };
		const UINT nodeMax = RNS_MAX;
		bool nodeSw[nodeMax] = { false };
		bool nodeHad[nodeMax] = { false };
		UINT nodeCount[nodeMax] = { 0 };
		vector<bool>& nodeIsOnly = g_pConf->m_aResSceneXMLNodeIsOnly;
		vector<wstring>& nodeName = g_pConf->m_aResSceneXMLNodeName;
		...
		POLL_XML_NODE_BEGIN(RNS_SCN_INF)
			POLL_XML_NODE_BEGIN(RNS_SCN_TBL)
				POLL_XML_NODE_BEGIN(RNS_SCN)
				POLL_XML_NODE_BEGIN(RNS_CMR_INF)
				POLL_XML_NODE_END(RNS_CMR_INF)
				POLL_XML_NODE_BEGIN(RNS_SPR_TBL)
					POLL_XML_NODE_BEGIN(RNS_SPR)
					POLL_XML_NODE_END(RNS_SPR)
				POLL_XML_NODE_END(RNS_SPR_TBL)
				POLL_XML_NODE_END(RNS_SCN)
			POLL_XML_NODE_END(RNS_SCN_TBL)
		POLL_XML_NODE_END(RNS_SCN_INF)

		return true;
	}

	bool readFromXsd(string xmlPath, string strRootNode)
	{
		map<string, void*> mapRoot;
