...
#pragma region 全局变量
TConfig* g_pConf = TConfig::GetInstance();
#pragma endregion
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	//控制台
	if (g_pConf->m_enDebug[TDEBUG_COM])
	{
		AllocConsole();
...
HRESULT WinApp::CreateDeviceIndependentResources()
{
	...
	//创建场景和精灵
	t_cout(TDEBUG_COM, endl << "Loading resource.");
	t_cout(TDEBUG_COM, endl << "Load resource finished.");
...
HRESULT WinApp::CreateDeviceResources()
{
	...
	if (!m_pRenderTarget)
	{
		...
		if (SUCCEEDED(hr))
		{
			SetTimer(m_hwnd, TMR_PAINT, 100, NULL);
		}
	}
